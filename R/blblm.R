...
#' \dontrun{
#' cl<-makeCluster(4)
#' par_blblm(mpg~hp,mtcars,10,100,cl,2)
#' stopCluster(cl)
#' }
#' @export
...
blbcoef <- function(fit,formula) {
  cf<-coef(fit)
  parm <- attr(terms(formula), "term.labels")
  labs<-c("int",parm)
  names(cf)<-labs
  cf
...
#' compute sigma from fit (not compatible with lm1C)
#'
#' @param fit the fitted model returned from blblm
#'
...
coef.blblm <- function(object, ...) {
  est <- object$estimates
  means<-map_mean(est, ~ map_rbind(., "coef") %>% colMeans()) #r col
   parm <- attr(terms(object$formula), "term.labels")
   labs<-c("(intercept)",parm)
   cf<-(means)
  names(cf)<-labs;(cf)
...
confint.blblm <- function(object, parm = NULL, level = 0.95, ...) {
  ...
  # y<-attr(terms(object$formula), "term.labels")
  # ps<-which(parm==y)+1
...
predict.blblm <- function(object, new_data, confidence = FALSE, level = 0.95, ...) {
  ...
  if (confidence) {
    ...
    map_mean(est, ~ map_cbind(., ~ X %*% .$coef[1,]) %>% rowMeans()) #rboind
  }
}
